it.only(" TC011 and TC012 Edit My Info", function () {
  ...
  cy.get(".oxd-calendar-selector-year")
    .click()
    .scrollIntoView({ duration: 300 }); // Select year
  cy.wait(2000);
  cy.get(":nth-child(33)").click(); // Pick specific year
  cy.get(".oxd-calendar-selector-month")
    .click()
    .scrollIntoView({ duration: 300 }); // Select month
  cy.wait(2000);
  cy.get(".oxd-calendar-dropdown > :nth-child(3)").click(); // Select day
  cy.wait(2000);
